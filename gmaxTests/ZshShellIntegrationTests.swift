...
struct ZshShellIntegrationTests {
    ...
    @Test func `install writes zsh wrapper files and shell integration snippet`() throws {
        ...
        let zprofile = try String(contentsOf: wrapperDirectory.appendingPathComponent(".zprofile"))
        ...
        #expect(zprofile.contains("typeset -g GMAX_SHELL_INTEGRATION_WRAPPER_ZDOTDIR=\"${ZDOTDIR:-}\""))
    ...
    @Test func `wrapper zprofile restores wrapper zdotdir after sourcing user zprofile`() throws {
        let fileManager = FileManager.default
        let rootDirectory = fileManager.temporaryDirectory
            .appendingPathComponent(UUID().uuidString, isDirectory: true)
        defer { try? fileManager.removeItem(at: rootDirectory) }
        ...
        #expect(zprofile.contains("typeset -g GMAX_SHELL_INTEGRATION_WRAPPER_ZDOTDIR"))
        #expect(zprofile.contains("export ZDOTDIR=\"${GMAX_SHELL_INTEGRATION_WRAPPER_ZDOTDIR}\""))
    }
